...
def get_cached_properties(name: str, version: str) -> dict | None:
    """
    ...
    with db_engine.connect() as connection:
        ...
        res = res.first()
        print(f'PROPS FOR {name} {version}: {res}')
...
def write_cache(name: str, version: str, hash: str, properties: dict) -> bool:
    ...
    write_package(name, version, hash)
    if not check_license_cache(hash):
        if properties["additional"] == "There are no additions.":
            properties["additional"] = ""

        write_license(hash, properties)
